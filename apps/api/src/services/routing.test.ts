...
describe('routing engine', () => {
  it('evaluates direct, multi-hop and split routes for XLM/USDC', () => {
    const routes = evaluateRoutes(source, 'XLM', 'USDC', 10000);
  ...
  it('ranks by net output, not headline fee', () => {
    const routes = evaluateRoutes(source, 'XLM', 'USDC', 10000);
  ...
  it('finds a multi-hop path where no direct pool exists', () => {
    const routes = evaluateRoutes(source, 'XLM', 'EURC', 500);
    assert.ok(routes.length > 0);
    assert.ok(routes.every((r) => r.outputAmount > 0));
    assert.ok(routes.every((r) => r.priceImpact >= 0));
  });

  it('returns no routes for unknown assets', () => {
    assert.deepEqual(evaluateRoutes(source, 'NOPE', 'USDC', 10), []);
    assert.equal(toQuote('NOPE', 'USDC', 10, []), null);
  });

  it('builds quote and routes responses from ranked routes', () => {
    const routes = evaluateRoutes(source, 'XLM', 'USDC', 10000);
